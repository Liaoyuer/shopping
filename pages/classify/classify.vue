<template>
	<view class="classify">
		<view class="head width-90 flex justify-between">
			<search class="search"></search>
			<u-icon @click="changeLayout" name="grid" size="52" color="#fa5376"></u-icon>
		</view>
		<view class="bottom flex justify-between">
			<view class="menu">
				<view class="menu-item active" v-for="(item,index) in categoryList" :key="index" :class="isActive==index?'menu-item-active':''" @click="selectMenu(index)">
					{{item.name}}
				</view>
			</view>
			<view class="goodsList">
				<view class="" v-if="goodsLayout">
					<view class="goodsList-top flex justify-end">
						<text>销量</text>
						<view class="iconup flex flex-direction justify-center">
							<u-icon name="arrow-up-fill" color="#999999" size="8"></u-icon>
							<u-icon name="arrow-down-fill" color="#999999" size="8"></u-icon>
						</view>
						<text>价格</text>
						<view class="iconup flex flex-direction justify-center">
							<u-icon name="arrow-up-fill" color="#999999" size="8"></u-icon>
					...
					<scroll-view :scroll-y="true" :scroll-with-animation="true" class="goodsList-bottom">
						<recommend :goodsList="goodsList" :pailie="pailie"></recommend>
					</scroll-view>
				</view>
				<view class="banner-box" v-else>
					<brand :classData="classData"></brand>
				</view>
			</view>
		</view>
	</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeLayout(){
				this.goodsLayout = !this.goodsLayout
			}
		},
		components:{
			search,
